...
public class BookServiceImpl implements BookService {
    ...
    public Book create(CreateBookRequest request, String isbn) {
        ...
        final List<CreateAuthorRequest> authorIds = request.getAuthors();
        List<Long> authorIdsLong = new ArrayList<>();

        for(CreateAuthorRequest authorRequest : request.getAuthors()) {
            List<Author> authorsFound = authorRepository.searchByNameName(authorRequest.getName());
            if( !authorsFound.isEmpty() ) {
                authorIdsLong.add(authorsFound.getFirst().getAuthorNumber());
            }
        }

        Book savedBook = create(isbn, title, description, photoURI, genre, authorIdsLong);
